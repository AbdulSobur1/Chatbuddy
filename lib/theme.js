...
const THEME_MODE_KEY = 'chatbuddy_theme_mode'
const ACCENT_COLOR_KEY = 'chatbuddy_accent_color'

// ─── Persistence helpers ───────────────────────────────────────
async function loadThemeMode() {
  try {
    if (Platform.OS === 'web') return localStorage.getItem(THEME_MODE_KEY)
...
async function saveThemeMode(mode) {
  try {
    if (Platform.OS === 'web') localStorage.setItem(THEME_MODE_KEY, mode)
    else await SecureStore.setItemAsync(THEME_MODE_KEY, mode)
  } catch { /* silent */ }
}

async function loadAccentColor() {
  try {
    if (Platform.OS === 'web') return localStorage.getItem(ACCENT_COLOR_KEY)
    return await SecureStore.getItemAsync(ACCENT_COLOR_KEY)
  } catch { return null }
}

async function saveAccentColor(hex) {
  try {
    if (Platform.OS === 'web') localStorage.setItem(ACCENT_COLOR_KEY, hex || '')
    else if (hex) await SecureStore.setItemAsync(ACCENT_COLOR_KEY, hex)
    else await SecureStore.deleteItemAsync(ACCENT_COLOR_KEY)
  } catch { /* silent */ }
}
...
export async function loadThemePreferences() {
  ...
  if (savedMode === 'dark' || savedMode === 'light' || savedMode === 'system') {
    ...
    _themeLoaded = true
    themeListeners.forEach((fn) => fn(savedMode))
  }
  const savedAccent = await loadAccentColor()
  if (savedAccent) {
    customAccentColor = savedAccent
    _accentLoaded = true
    accentListeners.forEach((fn) => fn(savedAccent))
  }
}
